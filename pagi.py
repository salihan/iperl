...
def pagination(df):
    ...
    gb.configure_columns([{'field': 'Current Total', 'cellStyle': JsCode(get_cell_style)}],
...
def lecturer_dashboard(df):
    ...
    with tab2:
        ...
        with st.container():
            ...
            # Display the paginated table -- worked
            # page_size = 20  # Set the number of rows per page
            # st.write(f"Displaying {len(styled_df)} rows")
            # start_idx = st.slider("Select starting row", 0, len(styled_df), step=page_size)
            # st.table(styled_df.iloc[start_idx:start_idx + page_size])

            # Set the styles for the 'Current Total' column using cellStyle
            styled_df = filtered_df[columns_to_display].copy().style.set_properties(**{
                'text-align': 'center'
            })

            page = pagination(styled_df.data)

            column_defs = [
                {'headerName': 'Skill', 'field': 'Skill'},
                {'headerName': 'Test 1', 'field': 'TEST1'},
                {'headerName': 'Continuous', 'field': 'CONTINUOUS'},
                {'headerName': 'Current Total', 'field': 'Current Total'},
                {'headerName': 'Final', 'field': 'FINAL'},
                {'headerName': 'Grade', 'field': 'GRADE'},
                {'headerName': 'Predicted Grade', 'field': 'PREDICTED GRADE'},
                {'headerName': 'Cognitive', 'field': 'Cognitive'},
                {'headerName': 'Psychomotor', 'field': 'Psychomotor'},
                {'headerName': 'Affective', 'field': 'Affective'},
            ]
